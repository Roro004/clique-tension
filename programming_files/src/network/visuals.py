...
def plot_graph(G, pos, old_pos, force_vectors, ax):
    ...
    pos_e = [(u, v) for u, v, d in G.edges(data=True) if G[u][v]['weight'] > 0]
    neg_e = [(u, v) for u, v, d in G.edges(data=True) if d.get('weight', 0) < 0]

    nx.draw_networkx_edges(G, pos, edgelist=pos_e, width=5, alpha=0.5, edge_color='green')
    nx.draw_networkx_edges(G, pos, edgelist=neg_e, width=5, alpha=0.5, edge_color='red')
